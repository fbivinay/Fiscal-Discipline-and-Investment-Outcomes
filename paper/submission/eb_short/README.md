...
**Current state: 10 pages total, 6 prose pages, 1 to spare** against the
seven-page limit — 8 tables and 1 figure, none of which count. Compiles with no
overfull boxes and no undefined references.

Figure 1 is generated by `code/make_region_figure.py`, which asserts the regional
mean debt figures it plots match the ones the text quotes (20.6, 26.2, 33.1,
34.5) before drawing anything. Re-run it if the panel changes.
...
limit, the budget binds only on prose. Anything that can be carried by a table or
a figure is therefore free, and the note uses that: the diagnostics, the
subsample robustness and the regional scatter all earn their place without
costing a line of the seven pages.
